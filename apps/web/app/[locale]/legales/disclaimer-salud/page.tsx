...
export async function generateMetadata({
  ...
  return {
    title: t("healthDisclaimer.metaTitle"),
  };
}
...
export default async function HealthDisclaimerPage({
  ...
  const t = await getTranslations({ locale, namespace: "legal" });
  const sections = t.raw("healthDisclaimer.sections") as LegalSection[];
  ...
  return (
    <main className="bg-bg min-h-screen px-6 py-16 max-w-[800px] mx-auto text-[#FAFAFA]">
      ...
      <div className="text-muted leading-[1.8] text-base">
        <p className="mb-6">{t("healthDisclaimer.intro")}</p>
